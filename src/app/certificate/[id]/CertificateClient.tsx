...
export default function CertificateClient({ moduleId }: { moduleId: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-6 md:py-10 print:bg-white print:py-0 print:min-h-0 flex flex-col items-center overflow-x-hidden">
      ...
      <div className="w-full max-w-[816px] flex flex-col sm:flex-row justify-between items-center mb-6 px-4 gap-4 print:hidden">
        <Button 
          type="button"
      ...
      {/* Actual Certificate Document Wrapper (Exact Letter Dimensions: aspect-[8.5/11]) */}
      <div className="w-full max-w-[800px] px-2 sm:px-4 mx-auto print:p-0 print:m-0 print:max-w-none">
        <div 
          className="certificate-paper [container-type:inline-size] w-full aspect-[8.5/11] bg-white text-brand-blue relative shadow-2xl mx-auto flex flex-col justify-between p-[3.5cqw] print:p-0 print:shadow-none print:aspect-auto"
        >
           {/* Internal Borders and Graphics */}
           <div className="certificate-paper-inner w-full h-full border-[2px] sm:border-[4px] md:border-[5px] border-double border-brand-blue/30 flex flex-col justify-between items-center p-[3.5cqw] text-center bg-white z-10 print:p-0">
              
              {/* Header: Auth Badges & Logo */}
              <div className="w-full flex justify-between items-center">
                 <div className="flex items-center gap-[1cqw] bg-brand-lightblue/10 px-[1.5cqw] py-[0.8cqw] rounded-lg border border-brand-blue/10 shadow-xs print:shadow-none print:border-none print:bg-transparent print:px-0">
                   <ShieldCheck className="h-[3.5cqw] w-[3.5cqw] text-brand-gold" />
                   <div className="text-left leading-tight">
                     <p className="text-[1.1cqw] font-bold text-slate-500 uppercase tracking-widest">Validación Oficial</p>
                     <p className="text-[1.4cqw] font-bold text-brand-blue font-mono tracking-wider">ID-{certId}</p>
                   </div>
                 </div>
                 <div className="flex items-center justify-end">
                   <Logo className="certificate-logo h-[4.8cqw] w-auto object-contain opacity-90 print:h-[9.5mm]" />
                 </div>
              </div>

              {/* Top Block: Award Icon + Title */}
              <div className="flex flex-col items-center my-auto pt-[1cqw]">
                <Award className="h-[6cqw] w-[6cqw] text-brand-blue/15 mb-[0.8cqw] block pointer-events-none" />
                <h1 className="text-[4.2cqw] font-headline font-bold tracking-tight text-brand-blue mb-[0.2cqw] uppercase leading-none">
                   Certificado
                </h1>
                <h2 className="text-[2.6cqw] font-headline font-light text-brand-green tracking-[0.25em] uppercase leading-snug">
                   de Aprobación
                </h2>
              </div>

              {/* Middle Block: Recipient + Course */}
              <div className="flex flex-col items-center my-auto max-w-[85%] py-[1cqw]">
                <p className="text-[1.3cqw] text-slate-500 uppercase tracking-[0.2em] mb-[1.5cqw] font-medium">
                   El presente documento formativo reconoce formalmente a
                </p>

                <h3 className="text-[3.8cqw] font-bold text-brand-blue font-headline mb-[1.5cqw] border-b-2 border-brand-lightblue/40 pb-[0.8cqw] inline-block px-[4cqw] uppercase tracking-wide leading-none">
                   {data.userName}
                </h3>

                <p className="text-[1.5cqw] text-slate-600 font-medium mb-[1.5cqw] leading-relaxed mx-auto px-[2cqw]">
                ...
                <h4 className="text-[2.2cqw] font-bold text-brand-green uppercase leading-snug">
                  "{data.moduleTitle}"
                </h4>
              </div>
              ...
              <div className="w-full flex justify-between items-end pt-[2cqw] border-t border-brand-blue/15 mt-auto">
                 <div className="text-center w-1/3 flex flex-col items-center justify-end">
                   <div className="relative mb-[0.4cqw]">
                     <span className="font-signature text-[3.4cqw] text-brand-blue font-bold tracking-wide transform -rotate-6 block select-none pointer-events-none drop-shadow-xs leading-none">
                       DiaCero
                     </span>
                   </div>
                   <div className="h-px w-[14cqw] bg-brand-blue/30 mx-auto mb-[0.8cqw]"></div>
                   <p className="text-[1.1cqw] font-bold text-brand-blue uppercase tracking-wider">Comité Evaluador</p>
                   <p className="text-[0.9cqw] text-slate-500 font-medium uppercase mt-[0.2cqw]">Plataforma Diacero</p>
                 </div>
                 
                 <div className="flex flex-col items-center justify-end w-1/3">
                   <div className="certificate-seal bg-amber-50 text-brand-gold border-2 border-brand-gold/40 rounded-full h-[9cqw] w-[9cqw] max-w-[68px] max-h-[68px] flex flex-col items-center justify-center p-[0.6cqw] shadow-xs">
                      <span className="text-[0.9cqw] font-bold uppercase tracking-wider opacity-80 mb-[0.2cqw] text-amber-800">Rendimiento</span>
                      <span className="text-[2.4cqw] font-bold tracking-tight text-amber-700 leading-none">{data.score}%</span>
                   </div>
                 </div>
                 ...
                 <div className="text-center w-1/3 flex flex-col items-center justify-end">
                   <a 
                     ...
                     className="inline-block mb-[0.4cqw] group cursor-pointer"
                   >
                     <img 
                       src={qrCodeUrl} 
                       alt="QR Sello Electrónico Formal" 
                       className="certificate-qr-img h-[6.5cqw] w-[6.5cqw] max-w-[58px] max-h-[58px] object-contain mx-auto group-hover:scale-105 transition-all duration-200" 
                     />
                   </a>
                   <div className="h-px w-[14cqw] bg-brand-blue/30 mx-auto mb-[0.8cqw]"></div>
                   <p className="text-[1.1cqw] font-bold text-brand-blue uppercase tracking-wider">Otorgado el {data.date}</p>
                   <a 
                     href={verifyUrl} 
                     target="_blank" 
                     rel="noopener noreferrer" 
                   ...
                   >
                     <p className="text-[0.9cqw] text-slate-500 font-medium uppercase mt-[0.2cqw]">Sello Electrónico Formal (ID-{certId})</p>
                   </a>
                 </div>
              </div>
